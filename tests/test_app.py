...
# url = "http://host.docker.internal:8000/api/v1/menus"
...
async def test_5(client):
    ...
    assert r.json()["description"] == var.description, "menu description is not valid"
    assert r.json()["title"] == "menu pytest", "menu title is not valid"
    assert r.json()["description"] == "menu pytest", "menu description is not valid"
...
async def test_13(client):
    ...
    assert r.json()["title"] == var.submenu_title, "submenu title is not valid"
    assert (
...
async def test_21(client):
    r = await client.get(url + "/" + var.id + "/submenus/" + var.submenu_id + "/dishes")
    assert r.status_code == 200, "wrong response"
    assert len(r.json()) == 0, 'table "dish" must be empty'


async def test_22(client):
    payload = {"title": "dish pytest", "description": "dish pytest", "price": "12.50"}
    r = await client.post(
        url + "/" + var.id + "/submenus/" + var.submenu_id + "/dishes", json=payload
    )
...
async def test_24(client):
    r = await client.get(
        url + "/" + var.id + "/submenus/" + var.submenu_id + "/dishes/" + var.dish_id
    )
    assert r.status_code == 200, "wrong response"
    assert r.json()["id"] == var.dish_id, "dish id is not valid"
    assert r.json()["title"] == var.dish_title, "dish title is not valid"
    assert r.json()["description"] == var.dish_description, "dish title is not valid"
    ...
    price = r.json()["price"]
    num = len(price[price.find(".") + 1 :])
    assert num == 2, "number of simbols after comma must be 2"
...
async def test_36(client):
    ...
    assert r.json()["dishes_count"] == var.menu_dish_count, "dishes_count is not valid"
...
async def test_37(client):
    ...
    assert r.status_code == 200, "wrong response"
    assert r.json()["id"] == var.submenu_id, "submenu id is not valid"
    assert r.json()["dishes_count"] == var.dish_count, "dishes_count is not valid"
